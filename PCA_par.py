...
class PCA_SVD:
    ...
    def multiply(self, A, B):
        with Pool(cpu_count()) as pool:
            result = pool.map(multiply_worker, [(A_row, B) for A_row in A])
        return result
    ...
    def covariance_matrix(self, X):
        ...
        with Pool(cpu_count()) as pool:
            results = pool.map(covariance_worker, [(i, j, X_transposed, n_samples) for i in range(len(X_transposed)) for j in range(len(X_transposed))])
    ...
    def svd(self, A):
        ...
        with Pool(cpu_count()) as pool:
            results = pool.map(power_iteration_worker, [(A, 100)] * len(A))
        for eigenvalue, eigenvector in results:
